          ...
          <div class="all-container">
              <div class="inner-all">
                <div class="devide-heading">
                     <span class="left-heading-word">Giao dịch</span>
                </div>
                  ...
                  <div class="under-list-table">
                      <div class="table-area">
                         <table class="table table-hover">
                            ...
                            <tbody>
                              <tr v-for="(i,index) in newData" v-bind:key="index">
                               <td>{{i.transition_id}}</td>
                               <td>{{i.money}}</td>
                               <td>{{i.bonus}}</td>
                               <td>{{i.type}}</td>
                               <td><span class="paid" v-if="i.status == 'thanh toán'" >{{i.status}}</span><span class="waiting-payment" v-else >{{i.status}}</span> </td>
                               <td>{{i.created_at}}</td>
                               <td><button class="process-but" @click="billBut(i.id)">chi tiết</button></td>
                              </tr>
                            </tbody>
                         </table>
                      </div>
 ...
 PoolView,
  ...
  methods:{
    showFirstlist(){
     ...
     getTransition(){
      axios.get('http://127.0.0.1:8000/api/transitionVulue').then(
        res=>{
...
.inner-all .devide-heading .left-heading-word{
  font-size: 20px; width: 110px; text-align: left; float: left; padding-top: 17px;
}
...
tr td .waiting-payment{color:#fff;background:#FFC107;font-size:13px;padding:3px 10px;border-radius:5px;}
tr td .paid{color:#fff;background:#28A745;font-size:13px;padding:3px 10px;border-radius:5px;}
</style>
